...
export default function MyPolls() {
  ...
  const handleDelete = async (e, id) => {
    ...
    if (
      window.confirm(
        "🗑️ Are you sure you want to delete this poll from your dashboard?",
      )
    ) {
      try {
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 bg-[#1e1f22] p-1 rounded-lg border border-[#3f4147]">
            <button
              ...
              className={`cursor-pointer px-3 py-1.5 text-xs font-bold rounded-md transition-all 
          ...
          {guilds.length > 0 && (
            <select
              ...
              border-[#3f4147] focus:border-[#5865F2] cursor-pointer"
            >
          ...
          >
            {polls.map((p) => (
              <div
                ...
                transition-all cursor-pointer group shadow-lg relative flex flex-col h-full"
              >
                <button
                ...
                <div className="space-y-3 text-sm pt-4 border-t border-[#3f4147] mt-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`px-2 py-1 rounded-md text-xs font-semibold 
                        ${p.is_active ? "bg-[#23a559]/10 text-[#23a559]" : "bg-[#404249] text-[#99AAB5]"}`}
                    >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[#43b581] font-bold flex items-center gap-1 truncate max-w-37.5">
                      ...
                      <span className="truncate">{p.channel_name}</span>
                    </span>
                  </div>
                </div>
              </div>
            ...
            {polls.length === 0 && (
              ...
              >
                ...
                <p className="text-[#99AAB5]">
                  No polls match your search or filters.
                </p>
              </div>
            )}
          </div>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-6 mt-10 pb-8">
              <button
                ...
                font-semibold text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#35373c] 
                transition-colors"
              >
                ← Previous
              </button>
              ...
              <button
                ...
                onClick={() => setPage((p) => p + 1)}
                className="cursor-pointer px-4 py-2 bg-[#2b2d31] border border-[#3f4147] rounded-md font-semibold 
                text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#35373c] transition-colors"
              >
                Next →
              </button>
            </div>
