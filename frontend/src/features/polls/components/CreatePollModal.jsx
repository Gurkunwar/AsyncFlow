...
export default function CreatePollModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
      <div className="bg-[#313338] w-full max-w-4xl rounded-xl border border-[#1e1f22] shadow-2xl flex 
      overflow-hidden min-h-125">
        ...
        <div className="w-1/3 bg-[#2b2d31] p-8 border-r border-[#1e1f22] hidden md:block">
          ...
          <div className="space-y-8 relative before:absolute before:inset-0 before:ml-3.75 before:-translate-x-px 
        ...
        <div className="flex-1 flex flex-col p-6 md:p-8 bg-[#313338] max-h-[90vh] overflow-y-auto custom-scrollbar">
          <div className="flex-1">
            ...
            {currentStep === 1 && (
              <div className="space-y-6 animate-fade-in">
                ...
                <div>
                  <label className="text-[11px] font-extrabold text-[#99AAB5] uppercase tracking-wider mb-2 flex 
                  ...
                  <div className="bg-[#1e1f22] rounded-md h-40 overflow-y-auto p-2 space-y-1 custom-scrollbar shadow-inner">
                    {isLoadingGuilds ? (
                      <div className="p-4 text-center text-[#99AAB5]">
                        Loading...
                      </div>
                    ) : (
                      guilds.map((g) => (
                        <div
                          ...
                          className={`flex items-center justify-between p-3 rounded-md transition-all ${
                            formData.guild_id === g.id
                              ...
                              : g.bot_present
                                ? "hover:bg-[#2b2d31] cursor-pointer border border-transparent"
                                : "opacity-60 border border-transparent"
                          }`}
                        >
                          <span
                            className={`text-sm font-semibold truncate ${g.bot_present ? "text-white" : "text-[#99AAB5]"}`}
                          >
                            {g.name}
                          </span>
                          ...
                          {!g.bot_present && (
                            <button
                              ...
                              className="cursor-pointer text-[10px] px-3 py-1.5 rounded font-bold uppercase 
                              bg-[#23a559] hover:bg-[#1d8a4a] 
                              text-white shadow-sm transition-transform active:scale-95"
                            >
                              Invite Bot
                            </button>
                          )}
            ...
            {currentStep === 2 && (
              <div className="space-y-6 animate-fade-in">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#99AAB5] mt-2">
                    Discord will automatically lock the poll after this time.
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4 animate-fade-in flex flex-col h-full">
                ...
                <div className="space-y-3">
                  {formData.options.map((opt, index) => (
                    <div key={index} className="flex items-center gap-3">
                      <div className="text-[#404249] font-bold w-4 text-center">
                        {index + 1}.
                      </div>
                      <input
                        type="text"
                      ...
                      {formData.options.length > 2 && (
                        <button
                          ...
                          className="cursor-pointer text-[#404249] hover:text-[#da373c] p-2 transition-colors"
                        >
                          ✕
                        </button>
                      )}
                ...
                {formData.options.length < 10 && (
                  <button
                    type="button"
                    onClick={addOption}
                    className="cursor-pointer text-sm text-[#5865F2] hover:text-[#4752C4] font-bold 
                    text-left pt-2 transition-colors"
                  >
                    + Add another option
                  </button>
                )}
          ...
          <div className="flex justify-between items-center pt-6 mt-6 border-t border-[#3f4147]">
            <button
              ...
              className="cursor-pointer px-6 py-2.5 rounded-md font-semibold text-sm text-[#99AAB5] 
            ...
            {currentStep < 3 ? (
              <button
                type="button"
                onClick={handleNext}
                className="cursor-pointer bg-[#5865F2] hover:bg-[#4752C4] px-8 py-2.5 rounded-md font-bold 
                text-sm text-white transition-all shadow-lg"
              >
                Continue
              </button>
            ) : (
              <button
                ...
                disabled={isCreating}
                className={`cursor-pointer bg-[#23a559] hover:bg-[#1d8a4a] px-8 py-2.5 rounded-md font-bold 
                  text-sm text-white transition-all shadow-lg ${isCreating ? "opacity-50" : ""}`}
              >
